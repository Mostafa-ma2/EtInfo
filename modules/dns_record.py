...
class dns_record:
    ...
    def dnsrec(self):
        ...
        print('\n' + colors.Yellow + '[!]' + colors.Yellow + ' Starting DNS Enumeration...' + colors.White + '\n')
        
        full_answer=[]
        
        for type in self.type.split(','):
            question = dnslib.DNSRecord.question(self.ip_address,type.upper())
            send_pkt = question.send('8.8.8.8',53,tcp='UDP')
            answer = dnslib.DNSRecord.parse(send_pkt)
            answer = str(answer)
            answer = answer.split('\n')
            full_answer.extend(answer)
            # answer=dns.resolver.query('sans.org',type)
            # full_answer.extend(answer)
        ...
        if len(dns_found) != 0:
            for entry in dns_found:
                print(colors.Green + '[+]' + colors.Cyan + ' {}'.format(entry) + colors.White)
        else:
            print(colors.Red + '[-]' + colors.Cyan + ' DNS Records Not Found!' + colors.White)
